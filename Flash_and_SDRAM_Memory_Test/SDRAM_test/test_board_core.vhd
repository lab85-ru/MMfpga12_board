...
begin
  ...
  ------------------------------------------------------------------------
  led <= "1001111" when progress = "00" else  -- "1" during initialization
         "0010010" when progress = "01" else  -- "2" when writing to memory
         "0000110" when progress = "10" else  -- "3" when reading from memory
         "0110000" when err = YES       else  -- "E" if memory test failed
         "0000001" when count=0         else  -- "O" if memory test passed
         "0000000" when count=200000000 else
			"0000001";
  ------------------------------------------------------------------------
  ...
  begin
    if(button_n='0')then
      ...
      divCnt <= divCnt+1;
		if(count<200000000 and progress="11")then
			count <= count+1;
		end if;
    end if;
  end process;
  ...
  heartBeat <= divCnt(16)               when progress/="11" else  -- test in progress
               ...
               divCnt(0)                when err = YES      else  -- test failed
               divCnt(16);					
